...
data class IncomingRequest(val id:String,val name:String,val size:Long,val relativePath:String,val checksumSha256:String)
...
class SeamReceiverServer(private val context:Context, private val identity:SeamIdentity, private val port:Int=38949){
    ...
    private fun handle(socket:Socket){socket.use{s->runCatching{
        ...
        if(request.startsWith("POST /request")){val body=readBody(input,headers["content-length"]?.toLongOrNull()?:0L);val o=org.json.JSONObject(body);val req=IncomingRequest(o.getString("id"),o.getString("name"),o.getLong("size"),o.getString("relativePath"),o.getString("checksum_sha256"));require(req.checksumSha256.matches(Regex("[0-9a-fA-F]{64}")));val future=CompletableFuture<Boolean>();pending[req.id]=future;onIncomingRequest?.invoke(req);val ok=runCatching{future.get(120,java.util.concurrent.TimeUnit.SECONDS)}.getOrDefault(false);pending.remove(req.id);respond(s,if(ok)200 else 403,if(ok)"OK" else "DECLINED");return@runCatching}
        ...
        val output=createOutput(rawName,rel) ?: run{respond(s,500,"unable to create output");return@runCatching};val digest=MessageDigest.getInstance("SHA-256");var remaining=length;var received=0L;var ok=false
        output.first.use{out->val buffer=ByteArray(256*1024);while(remaining>0){val n=input.read(buffer,0,minOf(buffer.size.toLong(),remaining).toInt());if(n<=0)break;out.write(buffer,0,n);digest.update(buffer,0,n);received+=n;remaining-=n}}
        ...
        ok=true;if(ok)onTransferVerified?.invoke(rawName,actual);respond(s,201,"VERIFIED")
    }}}
