...
class SeamTransferController(private val context:Context){
    ...
    fun send(uri:Uri,targetIp:String,targetPort:Int,token:String,relativePath:String?=null,onProgress:(Progress)->Unit):Result<Unit> = runCatching{
        ...
        val requestBody=JSONObject().apply{put("id",id);put("name",name);put("size",if(total>=0)total else 0);put("relativePath",rel)}.toString()
        ...
        val connection=(URL("http://$targetIp:$targetPort/receive").openConnection() as HttpURLConnection).apply{
            ...
            setRequestProperty("Content-Type","application/octet-stream");setRequestProperty("X-Seam-Token",token);setRequestProperty("X-File-Name",safeName);setRequestProperty("X-Relative-Path",safeRelative)
            if(total>=0)setFixedLengthStreamingMode(total)
        }
        resolver.openInputStream(uri).use{raw->requireNotNull(raw){"Unable to open selected file"};BufferedInputStream(raw).use{input->BufferedOutputStream(connection.outputStream).use{output->
