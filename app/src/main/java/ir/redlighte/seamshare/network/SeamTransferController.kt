...
class SeamTransferController(private val context: Context) {
    ...
    fun send(uri: Uri, targetIp: String, targetPort: Int, token: String, onProgress: (Progress) -> Unit): Result<Unit> {
        return runCatching {
            ...
            val connection = (URL("http://$targetIp:$targetPort/receive").openConnection() as HttpURLConnection).apply {
                requestMethod = "POST"; doOutput = true; connectTimeout = 5000; readTimeout = 30000
                setRequestProperty("Content-Type", "application/octet-stream")
                setRequestProperty("X-Seam-Token", token)
                setRequestProperty("X-File-Name", java.net.URLEncoder.encode(name, "UTF-8"))
            ...
            resolver.openInputStream(uri).use { input ->
                ...
                BufferedOutputStream(connection.outputStream).use { output ->
                    ...
                    while (input.read(buffer).also { n = it } != -1) {
                        output.write(buffer, 0, n); sent += n
                        val percent = if (total > 0) ((sent * 100) / total).toInt().coerceAtMost(100) else 0
                        onProgress(Progress(sent, total, percent))
                    }
                }
            }
            require(connection.responseCode in 200..299) { "Transfer failed: ${connection.responseCode}" }
